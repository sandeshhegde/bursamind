...
// Bursa numeric codes for TradingView (MYX uses 4-digit Bursa codes)
...
function TradingViewChart({ symbol }: { symbol: string }) {
  ...
  const code = TV_CODES[symbol] || symbol;
  const tvSymbol = `MYX:${code}`;
  ...
  return (
    <div>
      <div style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
        ...
        TradingView · {tvSymbol} · RSI + MACD included
      </div>
      <div ref={containerRef} style={{ height: 440, borderRadius: 10, overflow: "hidden" }} />
...
function TradingViewMini({ symbol }: { symbol: string }) {
  ...
  return <div ref={containerRef} style={{ borderRadius: 8, overflow: "hidden", minHeight: 220 }} />;
}
...
export default function StockPage() {
  ...
  // Company logo via Clearbit / fallback to initials
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <div>
              ...
              <div style={{ width: 52, height: 52, borderRadius: 14, overflow: "hidden", background: "linear-gradient(135deg, var(--card2), var(--border2))", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "1px solid var(--border)" }}>
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (parent) parent.innerHTML = `<span style="font-size:16px;font-weight:800;color:var(--accent)">${initials}</span>`;
                  }}
                />
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <span className="tag-gold">Bursa: {TV_CODES[symbol] || symbol}</span>
              <span className="tag-gold">TradingView: MYX:{TV_CODES[symbol]}</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div className="card" style={{ gridColumn: "span 2" }}>
            ...
            <table className="data-table">
              <thead><tr><th>Quarter</th><th>Revenue</th><th>Net Profit</th><th>EPS (sen)</th><th>Net Margin</th></tr></thead>
              <tbody>
                {financials.map((q, i) => (
                  <tr key={i}>
      ...
      {tab === "About" && (
        <div className="card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            <div>
              {[["Stock Code (Bursa)", TV_CODES[symbol] || symbol], ["Symbol (TradingView)", `MYX:${TV_CODES[symbol]}`], ["Exchange", "Bursa Malaysia Main Market"], ["Sector", stock.sector], ["Industry", stock.industry], ["Currency", "Malaysian Ringgit (MYR)"]].map(([k, v]) => (
            ...
            <div>
              {[["Market Cap", `MYR ${(stock.marketCap / 1000).toFixed(2)}B`], ["Shares Outstanding", `${(stock.shares / 1e9).toFixed(3)}B shares`], ["Fiscal Year End", "December 31"], ["Annual Revenue", `MYR ${(stock.revenue / 1000).toFixed(2)}B`], ["Annual Net Profit", `MYR ${(stock.netProfit / 1000).toFixed(2)}B`], ["Net Profit Margin", `${((stock.netProfit / stock.revenue) * 100).toFixed(1)}%`]].map(([k, v]) => (
                <div key={k} style={{ padding: "10px 0", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 12, color: "var(--text3)" }}>{k}</span>
                  <span style={{ fontSize: 12, color: "var(--text)", fontWeight: 600 }}>{v}</span>
                </div>
