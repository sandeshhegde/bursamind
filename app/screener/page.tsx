...
const SECTORS = ["All","Technology","Finance","Healthcare","Consumer Discretionary","Consumer Staples","Communication Services","Energy"];
type SK = keyof Stock;
export default function Screener() {
  ...
  return (
    <div className="fade-in">
      <div style={{marginBottom:24}}><h1 style={{fontSize:22,fontWeight:800,color:"var(--text)",marginBottom:4}}>Stock Screener</h1><p style={{fontSize:13,color:"var(--text3)"}}>Filter and sort US stocks (NYSE & NASDAQ) by fundamentals {isLive&&<span className="tag-green" style={{marginLeft:8}}>Live Prices</span>}</p></div>
      ...
      <div className="card" style={{padding:0}}>
        <div style={{padding:"14px 16px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between"}}><span style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{filtered.length} stocks found</span></div>
        <div style={{overflowX:"auto"}}>
          <table className="data-table"><thead><tr><th>Stock</th>{[{label:"Price",k:"price"},{label:"Chg %",k:"changePct"},{label:"Mkt Cap ($B)",k:"marketCap"},{label:"P/E",k:"pe"},{label:"P/B",k:"pb"},{label:"Div Yield",k:"dividendYield"},{label:"ROE %",k:"roe"},{label:"Volume",k:"volume"}].map(c=><th key={c.k} onClick={()=>handleSort(c.k as SK)} style={{cursor:"pointer",userSelect:"none"}}><span style={{display:"flex",alignItems:"center",gap:4}}>{c.label}<SI k={c.k as SK}/></span></th>)}</tr></thead>
          <tbody>{filtered.map(s=><tr key={s.symbol} onClick={()=>window.location.href=`/stock/${s.symbol}`} style={{cursor:"pointer"}}>
            ...
            <td className="mono">{s.pb?`${s.pb.toFixed(2)}x`:"—"}</td>
            <td className="mono">{s.dividendYield.toFixed(2)}%</td>
            <td className="mono">{s.roe?`${s.roe.toFixed(1)}%`:"—"}</td>
