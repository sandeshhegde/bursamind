...
export default function IPO() {
  ...
  return (
    <div className="fade-in" style={{maxWidth:1000}}>
      <div style={{marginBottom:24}}><h1 style={{fontSize:22,fontWeight:800,color:"var(--text)",marginBottom:4}}>US IPO Tracker</h1><p style={{fontSize:13,color:"var(--text3)"}}>Recent and upcoming IPOs on NYSE and NASDAQ</p></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:20}}>
        {[{label:"Total IPOs (2025)",value:IPO_LIST.length,color:"var(--blue)"},{label:"Listed",value:IPO_LIST.filter(i=>i.status==="Listed").length,color:"var(--accent)"},{label:"Upcoming",value:IPO_LIST.filter(i=>i.status==="Upcoming").length,color:"var(--gold)"}].map(s=><div key={s.label} className="card" style={{padding:"16px 18px"}}><div style={{fontSize:11,color:"var(--text3)",fontWeight:700,textTransform:"uppercase",marginBottom:8}}>{s.label}</div><div style={{fontSize:28,fontWeight:800,color:s.color}}>{s.value}</div></div>)}
      </div>
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {(["All","Listed","Upcoming"] as const).map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"7px 18px",borderRadius:8,fontSize:12,fontWeight:600,border:"none",cursor:"pointer",background:tab===t?"var(--accent)":"var(--card)",color:tab===t?"#000":"var(--text2)"}}>{t}</button>)}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {filtered.map(ipo=><div key={ipo.symbol} className="card" style={{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap"}}>
      ...
      <div style={{marginTop:20,padding:"14px 16px",background:"rgba(77,159,255,0.08)",border:"1px solid rgba(77,159,255,0.2)",borderRadius:10,display:"flex",gap:10,alignItems:"flex-start"}}>
        ...
        <div style={{fontSize:12,color:"var(--text3)",lineHeight:1.6}}>IPO data is for educational purposes. Always read the S-1 prospectus on SEC EDGAR before investing. Past IPO performance does not guarantee future results. Not financial advice.</div>
      </div>
    </div>
  );
}
