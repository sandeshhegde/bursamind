...
export default function Sectors() {
  ...
  const sectorStocks = selected
    ? STOCKS.filter(s => s.sector === selected)
    : [];

  const barData = SECTORS.map(s => ({ name: s.name.slice(0, 10), change: s.change }));

  return (
    <div className="fade-in" style={{ maxWidth: 1300 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", marginBottom: 4 }}>Sectors & Industries</h1>
        <p style={{ fontSize: 13, color: "var(--text3)" }}>Bursa Malaysia sector performance and breakdown</p>
      </div>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={barData} barSize={28}>
            ...
            <Tooltip
              ...
              formatter={(v: unknown) => { const n = v as number; return [`${n >= 0 ? "+" : ""}${n.toFixed(2)}%`, "Change"]; }}
            />
            <Bar dataKey="change" radius={[4, 4, 0, 0]}>
              {barData.map((d, i) => (
                <Cell key={i} fill={d.change >= 0 ? "var(--accent)" : "var(--red)"} fillOpacity={0.8} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        ...
        {SECTORS.map(s => (
          <div key={s.name}
            ...
            style={{
              ...
              border: selected === s.name ? "1px solid var(--accent)" : "1px solid var(--border)",
              background: selected === s.name ? "rgba(0,200,151,0.05)" : "var(--card)"
            }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>{s.name}</div>
                <div style={{ fontSize: 11, color: "var(--text3)" }}>{s.stocks} companies · MYR {(s.marketCap / 1000).toFixed(0)}B total</div>
              </div>
              <span className={s.change >= 0 ? "tag-green" : "tag-red"} style={{ fontSize: 13, padding: "4px 10px" }}>
      ...
      {/* Drilldown */}
      {selected && sectorStocks.length > 0 && (
        <div className="card fade-in" style={{ padding: 0 }}>
          ...
          <table className="data-table">
            <thead>
              <tr><th>Stock</th><th>Price (RM)</th><th>Change</th><th>Mkt Cap</th><th>P/E</th><th>Div Yield</th><th>ROE</th></tr>
            </thead>
            <tbody>
              {sectorStocks.map(s => (
                <tr key={s.symbol} onClick={() => window.location.href = `/stock/${s.symbol}`} style={{ cursor: "pointer" }}>
                  <td>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text3)" }}>{s.name.slice(0, 30)}</div>
                  </td>
