    ...
    <div className="fade-in" style={{maxWidth:1300}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
        {MARKET_INDICES.map(idx=><div key={idx.name} className="card" style={{padding:"16px 18px"}}>
          ...
          <div style={{fontSize:22,fontWeight:800,fontFamily:"Roboto Mono,monospace",color:"var(--text)",marginBottom:6}}>{idx.value.toLocaleString()}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        ...
        <div className="card">
          ...
          <div style={{display:"flex",alignItems:"center",gap:24}}>
            ...
            <div>{[{label:"Advancers",val:adv,color:"var(--accent)"},{label:"Decliners",val:dec,color:"var(--red)"},{label:"Unchanged",val:unch,color:"var(--text3)"}].map(b=><div key={b.label} style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><div style={{width:10,height:10,borderRadius:2,background:b.color,flexShrink:0}}/><div><div style={{fontSize:20,fontWeight:800,color:b.color,fontFamily:"Roboto Mono,monospace"}}>{b.val}</div><div style={{fontSize:11,color:"var(--text3)"}}>{b.label}</div></div></div>)}</div>
          </div>
        </div>
      </div>
      <div className="card" style={{marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {SECTORS.map(s=><div key={s.name} style={{padding:"14px 16px",borderRadius:10,background:s.change>=0?`rgba(0,200,151,${Math.min(0.05+Math.abs(s.change)*0.05,0.28)})`:`rgba(255,95,107,${Math.min(0.05+Math.abs(s.change)*0.05,0.28)})`,border:`1px solid ${s.change>=0?"rgba(0,200,151,0.2)":"rgba(255,95,107,0.2)"}`}}>
            ...
            <div style={{fontSize:18,fontWeight:800,fontFamily:"Roboto Mono,monospace"}} className={s.change>=0?"pos":"neg"}>{s.change>=0?"+":""}{s.change.toFixed(2)}%</div>
      ...
      <div className="card">
        ...
        <tbody>{CORPORATE_ACTIONS.map((a,i)=><tr key={i}><td style={{fontWeight:700,color:"var(--text)"}}>{a.company}</td><td><span className={a.action==="Dividend"?"tag-green":a.action==="Stock Split"?"tag-blue":"tag-gold"}>{a.action}</span></td><td>{a.detail}</td><td style={{fontFamily:"Roboto Mono,monospace",fontSize:12}}>{a.exDate}</td></tr>)}</tbody></table>
      </div>
    </div>
  );
}
